...
class ShreeListsScreens extends Component {
  componentDidMount() {
    this.fetchShreeCall();
  ...
  onSelectShree(params) {
    ...
    if (distance >2000) {
      HelperService.showToast({
        message: 'Cannot proceed. Dealer is too far from your location',
        duration: 2000
      });

      try {
        bugsnag.notify(new Error('Too Far Shree Dealer: ' + JSON.stringify({userId: userId, agent_lat_long: `${currentLocation.latitude}, ${currentLocation.longitude}`, party_name: params.data['Name'], party_id:params.id, party_lat_long: `${params.data['Latitude__c']}, ${params.data['Longitude__c']}`, distance: `${distance}m`})));
      }catch (error) {
        console.log(error)
      }
      return;
  ...
  render() {
    ...
    if (shreeList && shreeList.length) {
      ...
      visibleNode = <NoDataFound text={'No Dealers Found'} />
    }
...
const mapStateToProps = (state) => ({
  ...
  shreeList: state.shree.shreeDealersList,
  fetchShreeLoader: state.shree.fetchShreeLoader,
...
const mapDispatchToProps = (dispatch) => ({
  ...
  fetchShree: (params)               => dispatch(ShreeAction.fetchShree(params)),
