...
class LeadLostScreen extends Component {
    render() {
		const { 
			id,
			form,
...
const mapStateToProps = (state) => ({
	validation      			: state.leadAlerts.leadLostFormValidation,
	form 					 	: state.leadAlerts.leadLostForm,
	loader 			            : state.leadAlerts.loaders.markLeadLostLoader,
  	leadLostReasonsList 	    : state.common.leadLostReasonsList,
  
});
  
const mapDispatchToProps = (dispatch) => ({
	changeForm: (params)       => dispatch(LeadAlertsAction.changeLeadLostForm(params)),
	clearForm: ()              => dispatch(LeadAlertsAction.clearLeadLostForm())
});
